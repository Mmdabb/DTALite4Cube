...
def bus_delay(tst_link_performance_df, tfc_link_performance_df, link_df, route_df, assignment_period, output_file_name):
    ...
    tfc_link_performance_df['TT'] \
        = tfc_link_performance_df['length_in_mile'] / tfc_link_performance_df['speed']
    # unit in hours
    tfc_link_performance_df['delay'] \
        = tfc_link_performance_df['length_in_mile'] / tfc_link_performance_df['speed'] - tfc_link_performance_df['FFTT']
    ...
    for subnets_pair in net_pair_list:
        ...
        for subnet in subnets_pair:
            ...
            for time_period in time_periods:
                link_performance_df = pd.read_csv(os.path.join(subnet_dir, f'link_performance_{time_period.lower()}.csv'))
                link_df = pd.read_csv(os.path.join(subnet_dir,'link.csv'))
                link_df = link_df[['link_id', 'length_in_mile', 'free_speed']]
                
                route_mapping_df = pd.read_csv(os.path.join(regional_transit_stats_path, f'route_{time_period.lower()}.csv'))
                na_values = {'link_type_code': ['']}
                transit_stat_dtypes = {'link_type_name': 'str', 'link_type_code': 'str', 'density': 'float', 'TT_0': 'float'}
                transit_assignment = pd.read_csv(os.path.join(regional_transit_stats_path, f'static_link_performance_{time_period.lower()}.csv'), dtype=transit_stat_dtypes, na_values=na_values)
                
                assignment_period = f'{time_period.upper()}'
                transit_stat = bus_delay(transit_assignment, link_performance_df, link_df, route_mapping_df, assignment_period, subnet_name)
            ...
            if len(time_periods) > 1:
                ...
                subnet_transit_df[numeric_columns] = subnet_transit_df[numeric_columns].apply(pd.to_numeric, errors='coerce')


                total_mile_over_hour = subnet_transit_df.person_mile.sum() / np.maximum(subnet_transit_df.person_hour.sum(), 0.1)
                total_delay_over_hour = subnet_transit_df.person_delay.sum() / np.maximum(subnet_transit_df.person_hour.sum(), 0.1)
                ...
                total_subnet_transit_stats.append([subnet_name, 'Transit', 'total', 
                          ...
                          format(total_mile_over_hour, ".7f"), format(total_delay_over_hour, ".7f")
                             ])
...
if __name__ == "__main__":
    ...
    net_dir = r'C:\Users\mabbas10\Dropbox (ASU)\2. ASU\2. PhD\2. Projects\NVTA\3_Subarea_analysis\2024\2030 projects\Jan 31\gmns_nets\transit'
    regional_transit_stats_path = r'C:\Users\mabbas10\Dropbox (ASU)\2. ASU\2. PhD\2. Projects\NVTA\3_Subarea_analysis\2024\2030 projects\Jan 31\gmns_nets'
    
    
    
    sub_net_list = [item for item in os.listdir(net_dir) if os.path.isdir(os.path.join(net_dir, item)) and item != "statistics"]
