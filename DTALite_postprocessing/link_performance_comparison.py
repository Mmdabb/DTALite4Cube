...
def diff_stats(nb_link_per, nb_link, bd_link_per, bd_link, time_period, period_length, parent_dir):
    ...
    #district_id_dict = {'Arlington': 2, 'Alexandria': 1, 'Fairfax': 3, 'Fairfax City': 4, 'Falls Church': 5, 'Loudoun': 6, 'Prince William': 9, 'Manassas': 7, 'Manassas Park':8}
    
    district_id_dict = {2: 'Arlington', 1: 'Alexandria', 3: 'Fairfax', 4: 'Fairfax City', 5: 'Falls Church', 6: 'Loudoun', 9: 'Prince William', 7: 'Manassas', 8: 'Manassas Park'}

    

    bd_link_per['pair'] = bd_link_per['from_node_id'].astype(str) + '->' + bd_link_per['to_node_id'].astype(str)
    bd_link_per['FT'] = bd_link_per.link_type%10
    ...
    # unit in hours
    bd_link_per['TT'] = bd_link_per['length_in_mile'] / bd_link_per['speed']
    # unit in hours
    ...
    nb_link_per['FT'] = nb_link_per.link_type%10
    ...
    # unit in hours
    nb_link_per['TT'] = nb_link_per['length_in_mile'] / nb_link_per['speed']
    # unit in hours
    ...
    dif_bd = bd_df[['link_id','from_node_id','to_node_id','pair','volume','speed', 'person_delay','geometry','FT','TAZ','district_id']].copy()
    ...
    volume_nb_dict = dict(zip(nb_link_per.pair, nb_link_per.volume))
    speed_nb_dict = dict(zip(nb_link_per.pair, nb_link_per.speed))
    ...
    dif_bd['volume_nb'] = dif_bd.apply(lambda x: volume_nb_dict.setdefault(x.pair_bd, -1), axis=1)
    dif_bd['speed_nb'] = dif_bd.apply(lambda x: speed_nb_dict.setdefault(x.pair_bd, -1), axis=1)
    dif_bd['person_delay_nb'] = dif_bd.apply(lambda x: person_delay_nb_dict.setdefault(x.pair_bd, -1), axis=1)
    #dif_bd['mild_congestion_duration_nb'] = np.minimum(dif_bd.apply(lambda x: P_nb_dict.setdefault(x.pair_bd, -1), axis=1),period_length)
    #dif_bd['sever_congestion_duration_nb'] = np.minimum(dif_bd.apply(lambda x: sever_P_nb_dict.setdefault(x.pair_bd, -1), axis=1),period_length)
    #dif_bd['JUR_name'] = dif_bd.apply(lambda x: district_id_dict.setdefault(x.district_id_bd, -1), axis=1)        
    #diff = dif_bd.set_index('pair').join(dif_nb.set_index('pair'), how='right', lsuffix='_bd', rsuffix='_nb', sort=False)
            
                
    dif_bd['diff_volume']=dif_bd['volume_bd']-dif_bd['volume_nb']
    dif_bd['diff_speed']=dif_bd['speed_bd']-dif_bd['speed_nb']
    ...
    dif_bd.loc[dif_bd.speed_nb == -1 , "diff_speed"] = -1
    dif_bd.loc[dif_bd.volume_nb == -1 , "diff_volume"] = -1
    dif_bd.loc[dif_bd.volume_nb == -1 , "diff_person_delay"] = -1
...
def district_based_diff(net_dir_bd, net_dir_nb, parent_dir):
    ...
    for net_dir in net_list:
        ...
        if not os.path.exists(output):
            ...
            link_perf_net = link_perf_net_am[['link_id','from_node_id','to_node_id','speed','volume','person_volume',
            ...
            print('link performance statistics has been generated for the whole day and saved in the following directory: \n %s' %(net_dir))
    ...
    link_perf_net_bd['pair'] = link_perf_net_bd['from_node_id'].astype(str) + '->' + link_perf_net_bd['to_node_id'].astype(str)
    link_perf_net_bd['district_id'] = link_perf_net_bd.apply(lambda x: pair_district_dict_bd.setdefault(x.pair, -1), axis=1)
    ...
    link_perf_net_nb['pair'] = link_perf_net_nb['from_node_id'].astype(str) + '->' + link_perf_net_nb['to_node_id'].astype(str)
    link_perf_net_nb['district_id'] = link_perf_net_nb.apply(lambda x: pair_district_dict_nb.setdefault(x.pair, -1), axis=1)
    ...
    for district_name, district_id in district_id_dict.items():
        ...
        link_perf_jur_net_bd['length_in_mile'] = link_perf_jur_net_bd.apply(lambda x: length_dict_bd.setdefault(x.pair, -1), axis=1)
        link_perf_jur_net_bd['free_speed'] = link_perf_jur_net_bd.apply(lambda x: free_speed_dict_bd.setdefault(x.pair, -1), axis=1)
        ...
        link_perf_jur_net_nb['length_in_mile'] = link_perf_jur_net_nb.apply(lambda x: length_dict_nb.setdefault(x.pair, -1), axis=1)
        link_perf_jur_net_nb['free_speed'] = link_perf_jur_net_nb.apply(lambda x: free_speed_dict_nb.setdefault(x.pair, -1), axis=1)
        ...
        dif_bd[f'link_id_{case_nb}'] = dif_bd.apply(lambda x: link_id_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'from_node_id_{case_nb}'] = dif_bd.apply(lambda x: from_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'to_node_id_{case_nb}'] = dif_bd.apply(lambda x: to_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'volume_{case_nb}'] = dif_bd.apply(lambda x: volume_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'speed_{case_nb}'] = dif_bd.apply(lambda x: speed_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'person_delay_{case_nb}'] = dif_bd.apply(lambda x: phd_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
        dif_bd[f'person_mile_{case_nb}'] = dif_bd.apply(lambda x: pmt_nb_dict.setdefault(x[f'pair_{case_bd}'], -1), axis=1)
