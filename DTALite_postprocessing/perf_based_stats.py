    ...
    for subnet_list in new_net_list:
        ...
        for sub_net in subnet_list:
            ...
            header_df = ['subarea', 'time_period', 'delay', 'person_delay', 'person_hour', 'person_mile', 'mile/hour',
                         ...
                         'mean_severe_congestion_duration(length avg)',
                         'severe_congestion_duration(length weighted sum)',
                         'vehicle_mile', 'vehicle_hour', 'vehicle mile/hour',
                         'trk_vehicle_mile', 'trk_vehicle_hour', 'trk mile/hour',
                         'HOV/HOT_delay', 'HOV/HOT_person_delay', 'HOV/HOT_person_hour', 'HOV/HOT_person_mile',
                         'HOV/HOT_mile/hour', 'HOV/HOT_delay/hour'
                         ]
            ...
            for periods in time_periods:
                ...
                # link_net[f'VDF_allowed_uses_keys{period_number}'] = link_net[f'VDF_allowed_uses{period_number}'].map(
                #     reverse_allowed_uses_dict).fillna(-1)
                # Filter link_net based on conditions for keys 2 or 3
                hov_filtered_links = link_net[
                    (link_net['TOLLGRP'] == 2) |
                    (link_net[f'{periods.upper()}LIMIT'] == 2) |
                    (link_net[f'{periods.upper()}LIMIT'] == 3)
                    ]
                ...
                link_perf_net['pair'] = link_perf_net['from_node_id'].astype(str) + '->' + link_perf_net[
                ...
                old_vdf_code = link_perf_net.link_type / 100
                link_type_ft = (old_vdf_code % 10).astype(int)
                link_perf_net['FT'] = link_type_ft
                # link_perf_net.to_csv(os.path.join(net_dir, f'link_perf_net_{periods}.csv'), index=False)

                link_perf_jur_net = link_perf_net[
                    (link_perf_net['TAZ'] > 1404) & (link_perf_net['TAZ'] < 2820) & (link_perf_net['FT'] > 0)].copy()

                # link_perf_jur_net.to_csv(os.path.join(net_dir, f'link_perf_jur_net_{periods}.csv'), index=False)

                if link_perf_jur_net.vehicle_volume.sum() < 0.5 or link_perf_jur_net.empty:
                    continue

                link_perf_jur_net['length_in_mile'] = link_perf_jur_net.apply(
                    lambda x: length_dict.setdefault(x.pair, -1), axis=1)
                link_perf_jur_net['free_speed'] = link_perf_jur_net.apply(
                    lambda x: free_speed_dict.setdefault(x.pair, -1), axis=1)
                ...
                link_perf_jur_net['TT'] = link_perf_jur_net['length_in_mile'] / link_perf_jur_net['speed_mph']
                ...
                link_perf_jur_net['length_weighted_P'] = link_perf_jur_net['severe_congestion_duration_in_h'] * \
                                                         link_perf_jur_net['length_in_mile']

                link_perf_jur_net['vehicle_mile'] = link_perf_jur_net.vehicle_volume * link_perf_jur_net.length_in_mile
                link_perf_jur_net['vehicle_hour'] = link_perf_jur_net.vehicle_volume * link_perf_jur_net.TT
                vehicle_mile_over_hour = link_perf_jur_net.vehicle_mile.sum() / np.maximum(
                    link_perf_jur_net.vehicle_hour.sum(), 0.1)

                link_perf_jur_net[
                    'trk_vehicle_mile'] = link_perf_jur_net.vehicle_vol_trk * link_perf_jur_net.length_in_mile  # occ of truck = 1
                link_perf_jur_net['trk_vehicle_hour'] = link_perf_jur_net.vehicle_vol_trk * link_perf_jur_net.TT
                trk_vehicle_mile_over_hour = link_perf_jur_net.trk_vehicle_mile.sum() / np.maximum(
                    link_perf_jur_net.trk_vehicle_hour.sum(), 0.1)

                if link_perf_jur_net.person_hour.sum() < 0.5:
                    print('sum of person_hour for %s = %s' % (periods, link_perf_jur_net.person_hour.sum()))

                mile_over_hour = link_perf_jur_net.person_mile.sum() / np.maximum(link_perf_jur_net.person_hour.sum(),
                                                                                  0.1)
                delay_over_hour = link_perf_jur_net.person_delay.sum() / np.maximum(link_perf_jur_net.person_hour.sum(),
                                                                                    0.1)

                severe_P_length_weighted = link_perf_jur_net['length_weighted_P'].sum() / link_perf_jur_net[
                    'length_in_mile'].sum()
                ...
                hov_mile_over_hour = hov_link_perf_net.person_mile.sum() / np.maximum(
                    hov_link_perf_net.person_hour.sum(), 0.1)
                hov_delay_over_hour = hov_link_perf_net.person_delay.sum() / np.maximum(
                    hov_link_perf_net.person_hour.sum(), 0.1)
                ...
                period_stat.append([f'{sub_net}', f'{periods}',
                                    ...
                                    np.minimum(link_perf_jur_net['severe_congestion_duration_in_h'].max(),
                                               period_length),
                                    np.minimum(link_perf_jur_net['severe_congestion_duration_in_h'].mean(),
                                               period_length),
            ...
            if len(time_periods) > 1:
                ...
                total_stat.append([f'{sub_net}', 'total',
                                   ...
                                   per_perf_based_stats.person_mile.sum() / np.maximum(
                                       per_perf_based_stats.person_hour.sum(), 0.1),
                                   per_perf_based_stats.person_delay.sum() / np.maximum(
                                       per_perf_based_stats.person_hour.sum(), 0.1),
                                   ...
                                   per_perf_based_stats.vehicle_mile.sum() / np.maximum(
                                       per_perf_based_stats.vehicle_hour.sum(), 0.1),
                                   per_perf_based_stats.trk_vehicle_mile.sum(),
                                   per_perf_based_stats.trk_vehicle_hour.sum(),
                                   per_perf_based_stats.trk_vehicle_mile.sum() / np.maximum(
                                       per_perf_based_stats.trk_vehicle_hour.sum(), 0.1),
                                   ...
                                   per_perf_based_stats['HOV/HOT_person_mile'].sum() / np.maximum(
                                       per_perf_based_stats['HOV/HOT_person_hour'].sum(), 0.1),
                                   per_perf_based_stats['HOV/HOT_person_delay'].sum() / np.maximum(
                                       per_perf_based_stats['HOV/HOT_person_hour'].sum(), 0.1)
                                   ])
