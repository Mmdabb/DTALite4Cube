...
def demand_file_list(modes, period_titles, period_times, format_type='column'):
    demand_file_list_dict = {}
    for time_period, period_duration in zip(period_titles, period_times):
        file_seq_dict = {}
        file_sequence = 1
        for mode in modes:
            file_seq_dict[file_sequence] = [f'{mode}_{time_period}.csv', format_type, time_period.upper(), mode]
            file_sequence += 1
